...
func (l *DefaultLogger) FatalfIf(cond bool, fmt string, args ...interface{}) {
	if cond {
		l.sugarLogger.Fatalf(fmt, args)
	}
}

func (l *DefaultLogger) FatalIfError(err error) {
	if err != nil {
		l.sugarLogger.Fatalf("application fatal with %v", err.Error())
	}
}
